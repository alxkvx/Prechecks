...
from poaupdater import uConfig, uLogging, uSysDB, uPEM, uPrecheck, uUtil, openapi, uHCL, uBilling, uTasks
...
def cep_task():
	...
	task = cur.fetchone()[0]
	try:
		print 'rescheduling task id: %i ' % (task)
	...
	for file in glob.glob('/var/log/pa/update*'):
		with open(file, 'r') as searchfile:
			for line in searchfile:
				if 'Odin Automation has been successfully updated to version oa-7.0-9781.' in line:
					return file
...
def parse_upd_log():
	...
	print "this is upgrade log: %s\nchecking upgrade phases:\n" % file
	with open(file, 'r') as f:
		fline = f.readline()
		print "first line: %s" % fline
		for line in f:
...
def failed_tasks():
	...
	print "Number of Active Tasks:\t\t\t%s" % uTasks.getNumberOfActiveTasks()
	print "Number of Unprocessed Install Tasks:\t%s" % uTasks.getNumberOfUnprocessedInstallationTasks()
	print "Number of Failed Install Tasks:\t\t%s" % uTasks.getNumberOfFailedInstallationTasks()
	...
	if row is not None:
		...
		print "%s\t| %s%s | %s | %s" % (row[0],row[1],tab,row[2],row[3])
		for row in cur.fetchall():
			if len(str(row[1])) < 3: tab = '\t '
			print "%s\t| %s%s | %s | %s" % (row[0],row[1],tab,row[2],row[3])
	else:
		print "[  OK  ]\tNo failed install tasks."

	print "Failed tasks for the last day:\t\t",
